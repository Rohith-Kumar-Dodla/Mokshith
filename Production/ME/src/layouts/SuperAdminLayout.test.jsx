...
describe('SuperAdminLayout', () => {
  ...
  it('renders only one logout button', () => {
    ...
    expect(screen.getByRole('link', { name: /^Supplier Allocation$/i })).toBeInTheDocument();
    expect(screen.queryByRole('link', { name: /^Procurement$/i })).not.toBeInTheDocument();
    expect(screen.queryByRole('link', { name: /Payment Verifications/i })).not.toBeInTheDocument();
  });
});
