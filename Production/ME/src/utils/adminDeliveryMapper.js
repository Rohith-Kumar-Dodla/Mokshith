...
export function mapAdminDeliveryQueue(payload) {
  return unwrapList(payload).map((shipment) => {
    ...
    return {
      ...
      rejectionCount: Number(shipment.rejectionCount || 0),
      distanceKm: shipment.distanceKm ?? shipment.currentOfferId?.distance ?? null,
      distanceUnit: shipment.distanceUnit || shipment.currentOfferId?.distanceUnit || 'KM',
      deliveryAmount: shipment.deliveryAmount ?? shipment.currentOfferId?.deliveryAmount ?? null,
      currentOffer: shipment.currentOfferId || null,
